...
class AutomataGUI:
    ...
    def initUI(self):
        self.root.title("Regular Expressions to Finite Automata converter")
        ...
        padX = 10
        padY = 10
        self.root.geometry("{}x{}+{}+{}".format(self.FrameSizeX, self.FrameSizeY, int(FramePosX), int(FramePosY)))
        ...
        enterRegexLabel = Label(regexFrame,
                                text="Enter regular expression [operators allowed are plus (+), dot (.) and star (*)]:")
        ...
        self.automataCanvas = Canvas(automataCanvasFrame, bg='#FFFFFF', width=self.cwidth,
                                     height=self.cheight, scrollregion=(0, 0, self.cwidth, self.cheight))
    ...
    def handleBuildRegexButton(self):
        ...
        self.timingLabel.configure(text="")
    ...
    def handleTestStringButton(self):
        ...
        if self.dfaObj.acceptsString(inp):
            self.statusLabel.config(text="ACCEPTED")
        else:
            self.statusLabel.config(text="NOT ACCEPTED")
        self.timingLabel.configure(text="")
    ...
    # Save the PNG image if dot is found
        if self.dotFound:
            imagefile.save("automata.png", "PNG")
